addLayer("s", {
    ...
    upgrades: {
        ...
        15: {
            ...
            description: "Secret(SE) Upgrade! You can find it with weird requirement <br> x1~1.2 skills randomly every tick",
            tooltip() {if (hasUpgrade('c', 25)) 
                    {return "Requirement: Skills%10=7 <br> (1+0.2*(points%100/100))*2"}
            ...
            unlocked() {
                return (hasUpgrade('s',14) && (player.s.points % 10 == 7)) || hasUpgrade('s', 15)
            },
            effect() {
                let eff =(1+(0.2*(player.points%100/100)));
                if (hasUpgrade('c', 25)) {eff = eff*2};
                return eff
            },
            effectDisplay() { 
                return format(upgradeEffect(this.layer, this.id))+"x" 
...
addLayer("c", {
    ...
    upgrades: {
        ...
        25: {
            ...
            unlocked() {
                return hasUpgrade('s', 53) && player.s.points >= 1e8 && player.c.points <= (gain2*10) || hasUpgrade('c', 25)
            }
        }
    }
...
addLayer("g", {
    ...
    upgrades: {
        11: {
            ...
            tooltip: "Requirement: CP >= (CP boost)*1000<br>1+(log<sub>5</sub>(time<sup>0.1</sup>)",
            cost() {
                if ((inChallenge('g', 11) && (player.toky.points >= tmp.toky.gainMult.times(1000))) && tmp.toky.gainMult != 1) 
                {return new Decimal(0)}
                else {return new Decimal(Infinity)}
            },
            effect() {
                return (1+((Math.log(player.timese3.points**0.1))/Math.log(5)))
            },
            effectDisplay() { 
                return format(upgradeEffect(this.layer, this.id))+"x" 
            },
            unlocked() {
                return ((inChallenge('g', 11) && player.toky.points >= (tmp.toky.gainMult.times(1000))) && tmp.toky.gainMult != 1) || hasUpgrade('g', 11)
            }
